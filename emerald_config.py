...
def render_graph_VAODebug():
    ...
    SSAO = createPass('SSAO', {'enabled': True, 'kernelSize': 8, 'noiseSize': uint2(4,4), 'radius': 0.20000000298023224, 'distribution': SampleDistribution.Hammersley, 'depthMode': DepthMode.SingleDepth, 'guardBand': 64, 'thickness': 1.0})
    g.addPass(SSAO, 'SSAO')
...
def render_graph_VAONonInterleaved():
    ...
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    VAONonInterleaved = createPass('VAONonInterleaved', {'radius': 0.20000000298023224, 'primaryDepthMode': DepthMode.SingleDepth, 'secondaryDepthMode': DepthMode.StochasticDepth, 'exponent': 1.0, 'rayPipeline': False, 'guardBand': 64, 'thickness': 1.0})
...
def render_graph_HBAOPlusInterleaved():
    ...
    HBAOPlusInterleaved = createPass('HBAOPlusInterleaved', {'radius': 0.5, 'depthMode': DepthMode.SingleDepth, 'depthBias': 0.10000000149011612, 'exponent': 2.0})
...
def render_graph_VAORT():
    ...
    loadRenderPassLibrary('PathTracer.dll')
    loadRenderPassLibrary('RTAO.dll')
    VAORT = createPass('VAORT', {'radius': 0.20000000298023224, 'guardBand': 64, 'thickness': 1.0})
...
m.scene.camera.target = float3(-76.771164,4.926741,34.245533)
m.scene.camera.up = float3(0.000660,1.000000,0.000372)
