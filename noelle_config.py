...
def render_graph_VAODebug():
    ...
    SSAO = createPass('SSAO', {'enabled': True, 'kernelSize': 8, 'noiseSize': uint2(4,4), 'radius': 0.5, 'distribution': SampleDistribution.Hammersley, 'depthMode': DepthMode.StochasticDepth, 'guardBand': 64, 'thickness': 1.0})
    g.addPass(SSAO, 'SSAO')
...
def render_graph_VAONonInterleaved():
    ...
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    VAONonInterleaved = createPass('VAONonInterleaved', {'radius': 0.5, 'primaryDepthMode': DepthMode.SingleDepth, 'secondaryDepthMode': DepthMode.StochasticDepth, 'exponent': 1.0, 'rayPipeline': False, 'guardBand': 64, 'thickness': 1.0})
...
def render_graph_HBAOPlusInterleaved():
    ...
    HBAOPlusInterleaved = createPass('HBAOPlusInterleaved', {'radius': 1.100000023841858, 'depthMode': DepthMode.StochasticDepth, 'depthBias': 0.10000000149011612, 'exponent': 2.0})
...
# Scene
m.loadScene('D:/scenes/gltf/stairs_easy.gltf')
...
m.scene.camera.position = float3(4.173814,2.957911,3.480957)
m.scene.camera.target = float3(3.930222,2.567837,2.592979)
