...
def render_graph_VAODebug():
    ...
    StochasticDepthMap = createPass('StochasticDepthMap', {'SampleCount': 4, 'Alpha': 0.5, 'CullMode': CullMode.CullBack, 'linearize': True, 'depthFormat': ResourceFormat.D16Unorm})
...
def render_graph_VAONonInterleaved():
    ...
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    VAONonInterleaved = createPass('VAONonInterleaved', {'radius': 0.5, 'primaryDepthMode': DepthMode.SingleDepth, 'secondaryDepthMode': DepthMode.StochasticDepth, 'exponent': 1.600000023841858, 'rayPipeline': False})
...
def render_graph_HBAOPlusInterleaved():
    ...
    HBAOPlusInterleaved = createPass('HBAOPlusInterleaved', {'radius': 1.100000023841858, 'depthMode': DepthMode.SingleDepth, 'depthBias': 0.10000000149011612, 'exponent': 2.0})
...
def render_graph_VAORT():
    ...
    loadRenderPassLibrary('PathTracer.dll')
    loadRenderPassLibrary('RTAO.dll')
    VAORT = createPass('VAORT', {'radius': 0.5, 'guardBand': 64})
...
m.scene.camera.position = float3(4.173847,2.991163,3.481046)
m.scene.camera.target = float3(3.917389,2.732317,2.549796)
m.scene.camera.up = float3(0.000252,1.000000,0.000916)
