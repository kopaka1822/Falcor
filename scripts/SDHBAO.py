...
def render_graph_SDHBAO():
    g = RenderGraph('SDHBAO')
    ...
    g.create_pass('SDHBAO', 'SDHBAO', {'radius': 0.30000001192092896, 'depthMode': 'SingleDepth', 'depthBias': 0.10000000149011612, 'exponent': 2.0})
    ...
    g.add_edge('DeinterleaveDepth.texOut', 'SDHBAO.depth')
    g.add_edge('GBufferRaster.faceNormalW', 'SDHBAO.normals')
    g.add_edge('SDHBAO.ambientMap', 'InterleaveTexture0.texIn')
    g.mark_output('Ambient.out')
    g.mark_output('Diffuse.out')
    return g

SDHBAO = render_graph_SDHBAO()
try: m.addGraph(SDHBAO)
