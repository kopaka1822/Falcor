...
def render_graph_VAODebug():
    ...
    CrossBilateralBlur = createPass('CrossBilateralBlur', {'guardBand': 128})
    ...
    SSAO = createPass('SSAO', {'enabled': True, 'kernelSize': 8, 'noiseSize': uint2(4,4), 'radius': 0.20000000298023224, 'distribution': SampleDistribution.Hammersley, 'depthMode': DepthMode.SingleDepth, 'guardBand': 128, 'thickness': 0.800000011920929})
    g.addPass(SSAO, 'SSAO')
...
def render_graph_VAONonInterleaved():
    ...
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    VAONonInterleaved = createPass('VAONonInterleaved', {'radius': 0.20000000298023224, 'primaryDepthMode': DepthMode.SingleDepth, 'secondaryDepthMode': DepthMode.Raytraced, 'exponent': 1.1200000047683716, 'rayPipeline': False, 'guardBand': 128, 'thickness': 0.800000011920929})
...
def render_graph_HBAOPlusInterleaved():
    ...
    HBAOPlusInterleaved = createPass('HBAOPlusInterleaved', {'radius': 0.5, 'depthMode': DepthMode.StochasticDepth, 'depthBias': 0.10000000149011612, 'exponent': 2.0})
...
def render_graph_VAORT():
    ...
    loadRenderPassLibrary('PathTracer.dll')
    loadRenderPassLibrary('RTAO.dll')
    VAORT = createPass('VAORT', {'radius': 0.20000000298023224, 'guardBand': 128, 'thickness': 0.800000011920929})
...
def render_graph_HBAOPlusNoninterleaved():
    ...
    DepthPeelPass = createPass('DepthPeelPass')
    g.addPass(DepthPeelPass, 'DepthPeelPass')
    HBAOPlusNonInterleaved = createPass('HBAOPlusNonInterleaved', {'radius': 0.5, 'depthMode': DepthMode.StochasticDepth, 'depthBias': 0.10000000149011612, 'exponent': 2.0})
...
# Scene
m.loadScene('D:/scenes/orca/Bistro_v5_2/BistroExterior.pyscene')
