...
def render_graph_ML_HBAO():
    g = RenderGraph('ML_HBAO')
    ...
    ML_HBAO = createPass('ML_HBAO', {'enabled': True, 'radius': 0.5, 'guardBand': 64})
    g.addPass(ML_HBAO, 'ML_HBAO')
    ...
    g.addEdge('LinearizeDepth.linearDepth', 'ML_HBAO.depth')
    g.addEdge('GBufferRaster.faceNormalW', 'ML_HBAO.normals')
    g.addEdge('ML_HBAO.ambientMap', 'CrossBilateralBlur.color')
    g.addEdge('GBufferRaster.mtlData', 'MaterialDoubleSided.mtlData')
    g.addEdge('MaterialDoubleSided.doubleSided', 'ML_HBAO.doubleSided')
...
def render_graph_ML_HBAOInterleaved():
    ...
    DepthPass = createPass('DepthPass', {'depthFormat': ResourceFormat.D32Float, 'useAlphaTest': True, 'cullMode': CullMode.CullBack})
    g.addPass(DepthPass, 'DepthPass')
    ML_HBAOInterleaved = createPass('ML_HBAOInterleaved', {'enabled': True, 'radius': 0.5, 'guardBand': 64})
    g.addPass(ML_HBAOInterleaved, 'ML_HBAOInterleaved')
    MaterialDoubleSided = createPass('MaterialDoubleSided')
    g.addPass(MaterialDoubleSided, 'MaterialDoubleSided')
    g.addEdge('GBufferRaster.depth', 'LinearizeDepth.depth')
    g.addEdge('GBufferRaster.diffuseOpacity', 'Diffuse.I1')
    ...
    g.addEdge('LinearizeDepth.linearDepth', 'CrossBilateralBlur.linear depth')
    g.addEdge('DepthPass.depth', 'GBufferRaster.depth')
    g.addEdge('LinearizeDepth.linearDepth', 'ML_HBAOInterleaved.depth')
    g.addEdge('GBufferRaster.faceNormalW', 'ML_HBAOInterleaved.normals')
    g.addEdge('ML_HBAOInterleaved.ambientMap', 'CrossBilateralBlur.color')
    g.addEdge('GBufferRaster.mtlData', 'MaterialDoubleSided.mtlData')
    g.addEdge('MaterialDoubleSided.doubleSided', 'ML_HBAOInterleaved.doubleSided')
    g.markOutput('Ambient.out')
    g.markOutput('Diffuse.out')
    return g
...
def render_graph_RTHBAO():
    ...
    RTHBAO = createPass('RTHBAO', {'enabled': True, 'kernelSize': 8, 'radius': 0.5, 'depthMode': DepthMode.Raytraced, 'guardBand': 64, 'thickness': 0.0})
    g.addPass(RTHBAO, 'RTHBAO')
    ...
    g.addEdge('LinearizeDepth_.linearDepth', 'RTHBAO.depth2')
    g.addEdge('LinearizeDepth_', 'RTHBAO')
    g.addEdge('LinearizeDepth.linearDepth', 'RTHBAO.depth')
    g.addEdge('StochasticDepthMap.stochasticDepth', 'RTHBAO.stochasticDepth')
    g.addEdge('RTHBAO.ambientMap', 'ConvertFormat__.I0')
    g.addEdge('ConvertFormat__.out', 'CrossBilateralBlur.color')
    g.addEdge('RTHBAO', 'ConvertFormat__')
...
def render_graph_VAO():
    ...
    StochasticDepthMap = createPass('StochasticDepthMap', {'SampleCount': 4, 'Alpha': 0.20000000298023224, 'CullMode': CullMode.CullBack, 'linearize': True, 'depthFormat': ResourceFormat.D24UnormS8})
    g.addPass(StochasticDepthMap, 'StochasticDepthMap')
    VAO = createPass('VAO', {'enabled': True, 'kernelSize': 8, 'noiseSize': uint2(4,4), 'radius': 0.5, 'distribution': SampleDistribution.VanDerCorput, 'depthMode': DepthMode.Raytraced, 'guardBand': 64, 'thickness': 0.0})
    g.addPass(VAO, 'VAO')
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    DepthPass = createPass('DepthPass', {'depthFormat': ResourceFormat.D32Float, 'useAlphaTest': True, 'cullMode': CullMode.CullBack})
...
def render_graph_SVAO():
    ...
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    SVAO = createPass('SVAO', {'radius': 0.5, 'primaryDepthMode': DepthMode.SingleDepth, 'secondaryDepthMode': DepthMode.StochasticDepth, 'exponent': 1.0, 'rayPipeline': False, 'guardBand': 64, 'thickness': 0.0})
    g.addPass(SVAO, 'SVAO')
    DepthPass = createPass('DepthPass', {'depthFormat': ResourceFormat.D32Float, 'useAlphaTest': True, 'cullMode': CullMode.CullBack})
    g.addPass(DepthPass, 'DepthPass')
    g.addEdge('GBufferRaster.depth', 'DepthPeelPass.depth')
    ...
    g.addEdge('DepthPass.depth', 'GBufferRaster.depth')
    g.addEdge('GBufferRaster.depth', 'SVAO.gbufferDepth')
    g.addEdge('SVAO.ao', 'ConvertFormat__.I0')
    g.markOutput('Ambient.out')
    g.markOutput('Diffuse.out')
    return g
