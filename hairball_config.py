...
def render_graph_VAODebug():
    ...
    CrossBilateralBlur = createPass('CrossBilateralBlur', {'guardBand': 100})
    ...
    SSAO = createPass('SSAO', {'enabled': True, 'kernelSize': 8, 'noiseSize': uint2(4,4), 'radius': 0.10000000149011612, 'distribution': SampleDistribution.Hammersley, 'depthMode': DepthMode.StochasticDepth, 'guardBand': 100})
    g.addPass(SSAO, 'SSAO')
...
def render_graph_VAONonInterleaved():
    ...
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    VAONonInterleaved = createPass('VAONonInterleaved', {'radius': 0.10000000149011612, 'primaryDepthMode': DepthMode.SingleDepth, 'secondaryDepthMode': DepthMode.StochasticDepth, 'exponent': 1.600000023841858, 'rayPipeline': False, 'guardBand': 100})
...
def render_graph_HBAOPlusInterleaved():
    ...
    HBAOPlusInterleaved = createPass('HBAOPlusInterleaved', {'radius': 0.30000001192092896, 'depthMode': DepthMode.SingleDepth, 'depthBias': 0.10000000149011612, 'exponent': 2.0})
...
def render_graph_VAORT():
    ...
    loadRenderPassLibrary('PathTracer.dll')
    loadRenderPassLibrary('RTAO.dll')
    VAORT = createPass('VAORT', {'radius': 0.10000000149011612, 'guardBand': 100})
...
m.scene.camera.position = float3(4.715004,3.823052,6.848525)
m.scene.camera.target = float3(3.956582,3.677185,7.483756)
m.scene.camera.up = float3(0.000592,1.000000,-0.000496)
