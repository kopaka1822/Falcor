...
def render_graph_VAODebug():
    ...
    StochasticDepthMap = createPass('StochasticDepthMap', {'SampleCount': 4, 'Alpha': 0.5, 'CullMode': CullMode.CullBack, 'linearize': True, 'depthFormat': ResourceFormat.D24UnormS8})
    g.addPass(StochasticDepthMap, 'StochasticDepthMap')
    SSAO = createPass('SSAO', {'enabled': True, 'kernelSize': 8, 'noiseSize': uint2(4,4), 'radius': 0.15000000596046448, 'distribution': SampleDistribution.Hammersley, 'depthMode': DepthMode.SingleDepth, 'guardBand': 64, 'thickness': 0.30000001192092896})
    g.addPass(SSAO, 'SSAO')
...
def render_graph_VAONonInterleaved():
    ...
    ConvertFormat__ = createPass('ConvertFormat', {'formula': 'float4(I0[xy].x + I0[xy].y + I0[xy].z,0,0,0)', 'format': ResourceFormat.R8Unorm})
    g.addPass(ConvertFormat__, 'ConvertFormat__')
    VAONonInterleaved = createPass('VAONonInterleaved', {'radius': 0.15000000596046448, 'primaryDepthMode': DepthMode.SingleDepth, 'secondaryDepthMode': DepthMode.StochasticDepth, 'exponent': 1.4200000762939453, 'rayPipeline': False, 'guardBand': 64, 'thickness': 0.30000001192092896})
    g.addPass(VAONonInterleaved, 'VAONonInterleaved')
    DepthPass = createPass('DepthPass', {'depthFormat': ResourceFormat.D32Float, 'useAlphaTest': True, 'cullMode': CullMode.CullBack})
    g.addPass(DepthPass, 'DepthPass')
    StochasticDepthMap_ = createPass('StochasticDepthMap', {'SampleCount': 4, 'Alpha': 0.5, 'CullMode': CullMode.CullBack, 'linearize': True, 'depthFormat': ResourceFormat.D24UnormS8})
    ...
    g.markOutput('VAONonInterleaved.accessStencil')
    return g
...
def render_graph_HBAOPlusInterleaved():
    ...
    HBAOPlusInterleaved = createPass('HBAOPlusInterleaved', {'radius': 0.30000001192092896, 'depthMode': DepthMode.StochasticDepth, 'depthBias': 0.10000000149011612, 'exponent': 2.0})
...
def render_graph_VAORT():
    ...
    loadRenderPassLibrary('RTAODenoiser.dll')
    loadRenderPassLibrary('PassLibraryTemplate.dll')
    VAORT = createPass('VAORT', {'radius': 0.5, 'guardBand': 64, 'thickness': 1.0})
...
# Scene
m.loadScene('D:/scenes/models/hairball_desert/desert.gltf')
...
m.scene.camera.position = float3(4.877271,3.933204,6.743309)
m.scene.camera.target = float3(4.048779,3.730328,7.265271)
